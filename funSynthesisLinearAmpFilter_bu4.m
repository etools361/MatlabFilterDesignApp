...
%--------------------------------------------------------------------------
function [cellValueNetlist, km, Rs, Rl] = funSynthesisLinearAmpFilter(n, Rs, Rl, fp, fs, Ap, As)
    if isempty(Ap) || Ap<0
        Ap = 3;
        fprintf('Ap=%f dB\n', Ap);
    end
    ...
    [cellValueNetlist, km, Rs, Rl] = funEvenOrderParameter(n, Rs, Rl, Ap);

function [cellValueNetlist, km, Rs, Rl] = funEvenOrderParameter(n, Rs, Rl, Ap)
    if n>10
        digits(40)
    else
        digits(32)
