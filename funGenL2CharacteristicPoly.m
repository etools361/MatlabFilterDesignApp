...
% 注意返回值低次在前，高次在后
%--------------------------------------------------------------------------
function [Ln] = funGenL2CharacteristicPoly(FilterOrder)
...
for ii = 1:half_order + 1
    a = -0.5;% chebyshev
    nn = ii - 1;
    current_coefficient = coefficients(ii);
    legendrePoly = 2.^(2*nn)./(nchoosek(2*nn, nn)).*JacobiPoly(nn, a, a); % Gen legendre polynomial
